...
const auctionListApi = async (
  ...
  if (item_name?.length === 0 && auction_item_category?.length === 0)
    ...
    url += `auction_item_category=${auction_item_category}`;
  else
    url += `auction_item_category=${auction_item_category}&item_name=${item_name}`;

  if (cursor.length > 0) url += `&cursor=${cursor}`;

  try {
    const response: AxiosResponse<ResponseAuctionList> =
      await nexonApiAxiosInstance.get(url);
...
const auctionHistoryApi = async (
  ...
  try {
    const response: AxiosResponse<ResponseAuctionHistory> =
      await nexonApiAxiosInstance.get(url);
    return response.data;
  } catch (e) {
    if (e instanceof AxiosError) {
...
export { auctionListApi, auctionHistoryApi };
